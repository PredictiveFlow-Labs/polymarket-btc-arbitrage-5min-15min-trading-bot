...
class RiskManager {
    constructor(config, initialBankroll) {
        this.config = config;
        this.log = (0, logger_1.getLogger)().child({ module: 'RiskManager' });
    ...
    canTrade(marketId) {
        ...
        if (this.state.dailyPnl <= -dailyStopAmount) {
            ...
            this.log.warn({ dailyPnl: this.state.dailyPnl, limit: -dailyStopAmount }, 'daily stop loss hit');
            return { ok: false, reason: 'DAILY_LOSS_LIMIT' };
        }
        return { ok: true };
    }
    // ── Position sizing ───────────────────────────────────────────────────────
    sizePosition(strength) {
    ...
    openPosition(position) {
        ...
        this.log.info({ marketId: position.marketId, side: position.side, amount: position.amount }, 'position opened');
    }
    closePosition(marketId, exitPrice) {
        ...
        this.log.info({ marketId, side: pos.side, pnl: pnl.toFixed(4), bankroll: this.state.bankroll.toFixed(2) }, 'position closed');
        return pos;
    }
    expirePosition(marketId, won) {
        const pos = this.openPositions.get(marketId);
        if (!pos)
            return null;
        ...
        this.state.bankroll += pnl;
        this.state.dailyPnl += pnl;
        this.log.info({ marketId, won, pnl: pnl.toFixed(4) }, 'position expired/settled');
        return pos;
    }
