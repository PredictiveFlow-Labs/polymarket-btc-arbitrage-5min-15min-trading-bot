...
export class SniperStrategy {
  ...
  async executeSignal(signal: SniperSignal, market: ParsedMarket): Promise<ActivePosition | null> {
    ...
    try {
      ...
      const placementCtx: TradePlacementContext = {
        slug: market.slug,
      ...
      // Place limit order
      const trade = await this.execution.placeTrade(
        market.id,
        signal.side,
        sizing.amount,
        this.config.dryRun,
  ...
  async closeBeforeExpiry(position: ActivePosition, market: ParsedMarket): Promise<void> {
    ...
    try {
      await this.execution.placeTrade(
        position.marketId,
        exitSide,
        position.amount,
        this.config.dryRun,
